...
void *other_thread_func(void *arg) {
	...
	chronosrt_set_tdf(0.5);
	int section_id = chronosrt_enter_critical();
	timeline_timestamp("entered critical section\n");

	timeline_spin_for(1000);

	timeline_timestamp("about to exit critical section\n");
	timeline_event_happened(&ev_cnt, 2);
	chronosrt_exit_critical(section_id);

	timeline_timestamp("about to detatch\n");
	chronosrt_detatch();
	printf("other thread done\n");
	timeline_event_happened(&ev_cnt, 4);
	return NULL;
}
...
int main() {
	...
	timeline_timestamp("started on work\n");
	timeline_spin_for(1000);
	timeline_timestamp("finished work\n");
	timeline_event_happened(&ev_cnt, 3);

	timeline_timestamp("waiting for other thread\n");
	pthread_join(other_thread, NULL);
	timeline_timestamp("joined other thread\n");
